...
const BookList = () => {
    ...
    const fetchBookList = async (val) => {
        ...
        if (data.next) {
            // const originalUrl = data.next;
            // const domainUrl = 'skunkworks.ignitesol.com';

            // const replacedUrl = originalUrl.replace(/^https?:\/\/[^\/]+/, `http://${domainUrl}`);
    ...
    const renderLatestCardData = () => {
        return (
            <div className="row" style={{ height: "500px" }} >
                {cardData.map((item) => {
                    return (
                        <div className='col-12 col-sm-2  book_container' onClick={() => {
        ...
        <div>
            <div className="container mt-50">
                ...
                <label className="searchLabelWrap">
                    ...
                    <input type="search" placeholder="Search" className="searchInput" onChange={(e) => {
